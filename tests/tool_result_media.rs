...
fn responses_text_stays_a_string_and_media_becomes_the_array() {
    let b = build("openai", "m", result(json!([{"type": "text", "text": "sunny"}]))).unwrap();
    assert_eq!(b["input"][2], json!({"type": "function_call_output", "call_id": "call_1", "output": "sunny"}));
    let b = build("openai", "m", result(json!([{"type": "text", "text": "panel"}, image()]))).unwrap();
    assert_eq!(
        b["input"][2]["output"],
...
fn responses_two_results_keep_their_ids_and_order() {
    ...
    let b = lm.build_request(&Request::from_json(&value).unwrap(), false).unwrap().body.unwrap();
    let items: Vec<(&str, usize)> = b["input"].as_array().unwrap()[3..]
        .iter()
        .map(|i| (i["call_id"].as_str().unwrap(), i["output"].as_array().unwrap().len()))
        .collect();
...
fn chat_default_rejects_and_a_proven_preset_sends_the_array() {
    let err = refuses(build("openai-chat", "m", result(json!([image()]))), "text-only tool results");
    assert!(err.message().contains("Responses"), "{err}");
    let b = build("xai", "m", result(json!([{"type": "text", "text": "panel"}, image()]))).unwrap();
    assert_eq!(
        b["messages"][2]["content"],
        json!([{"type": "text", "text": "panel"}, {"type": "image_url", "image_url": {"url": format!("data:image/png;base64,{PNG}")}}])
    );
    refuses(
        build("xai", "m", result(json!([{"type": "document", "media_type": "application/pdf", "data": PNG}]))),
        "carries images but not document",
    );
    refuses(build("groq", "m", result(json!([image()]))), "tool_result_media=\"reject\"");
}
...
fn anthropic_native_blocks_flag_and_reject_preset() {
    ...
    let kinds: Vec<&str> = block["content"].as_array().unwrap().iter().map(|c| c["type"].as_str().unwrap()).collect();
    assert_eq!(kinds, vec!["text", "image", "document"]);
    refuses(build("deepseek-anthropic", "deepseek-v4-flash", result(json!([image()]))), "text-only tool results");
    let _ = AnthropicLM::builder();
}

#[test]
fn gemini_nests_media_resolves_names_and_maps_is_error() {
    let b = build("gemini", "m", result(json!([{"type": "text", "text": "panel"}, image()]))).unwrap();
    ...
    assert_eq!(b["contents"][2]["parts"][0]["functionResponse"]["response"], json!({"error": "boom"}));
...
fn stop_and_top_k_raise_on_responses_and_top_k_on_chat() {
    ...
    let lm = OpenAILM::builder().api_key("k").build().unwrap();
    refuses(lm.build_request(&Request::from_json(&value).unwrap(), false).map(|r| r.body.unwrap()), "config.stop has no field");
    let value = json!({"model": "m", "messages": [{"role": "user", "parts": [{"type": "text", "text": "x"}]}], "config": {"top_k": 3}});
    refuses(lm.build_request(&Request::from_json(&value).unwrap(), false).map(|r| r.body.unwrap()), "config.top_k has no field");
...
fn build_request_validates_an_edited_request() {
    ...
    let lm = OpenAILM::builder().api_key("k").build().unwrap();
    let mut request = Request::from_json(&canonical("m", result(json!([{"type": "text", "text": "x"}])))).unwrap();
